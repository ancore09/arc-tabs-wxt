...
- Список закрепленных групп позволяет перемещать группы - такоим образом указывается их порядок. Предусмотрены кнопки для перемещения группы наверх и вниз списка.
